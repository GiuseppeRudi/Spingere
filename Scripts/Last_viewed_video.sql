/*select v.Username, vid.Titolo as 'Ultimo contenuto visto', v.`Data`
from visualizzare v, video vid
where (v.`Data`>=all(select v2.`Data` from visualizzare v2 where v.Username=v2.Username) and v.Cod_video =vid.Cod_video)
group by v.Username
*/
